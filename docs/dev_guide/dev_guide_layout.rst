   ...
   rms-metadata-tools/
     ...
     src/
       metadata_tools/         # the importable engine (public package)
         ...
         index_support/        # index engine (package)
...
The index engine is a package:

.. code-block:: text

   src/metadata_tools/index_support/
     __init__.py        # re-exports the public surface (IndexTable, key functions, entry points)
     process.py         # get_args() and process_index() (the stage entry point)
     table.py           # IndexTable: builds one volume's supplemental index table
     key_fns.py         # built-in key__<NAME> column functions
