...
#===============================================================================
def test_planet_masker_ors_in_back(exists_true, fake_backplane):
...
def test_ring_masker_only_for_saturn(exists_true, fake_backplane):
...
def test_moon_blocker_masker(exists_true, fake_backplane):
...
def test_target_cannot_block_itself(exists_true, fake_backplane):
...
def test_ignore_shadows_skips_shadowers_and_faces(exists_true, fake_backplane):
...
def test_day_face_masks_antisunward(exists_true, fake_backplane):
    fake_backplane.antisunward['IO'] = np.ones((4, 4), dtype=bool)
    result = masks.construct_excluded_mask(
...
def test_shadowers_applied_when_not_ignored(exists_true, fake_backplane):
...
def test_pluto_primary_also_masks_charon(exists_true, fake_backplane):
...
def test_ring_shadower_for_saturn(exists_true, fake_backplane):
...
def test_moon_shadower(exists_true, fake_backplane):
...
def test_pluto_shadower_also_shadows_charon(exists_true, fake_backplane):
...
def test_night_face_masks_sunward(exists_true, fake_backplane):
...
def test_all_false_returns_python_false(exists_true, fake_backplane):
    result = masks.construct_excluded_mask(
        fake_backplane, 'IO', 'JUPITER', ('P', '', ''), ignore_shadows=True)
    assert result is False


def test_nonexistent_target_returns_true(monkeypatch, fake_backplane):
