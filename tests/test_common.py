...
# Table.__init__
#===============================================================================
def test_table_filename_default_suffix(tmp_path):
...
# Table.write
#===============================================================================
def test_write_empty_rows_returns_early(monkeypatch, tmp_path):
    table = com.Table(output_dir=FCPath(tmp_path), volume_id='GO_0001',
                      qualifier='body', level='summary')
    wrote = []
    monkeypatch.setattr(com.util, 'write_txt_file',
                        lambda *a, **k: wrote.append(a))
    monkeypatch.setattr(com.lab, 'create', lambda *a, **k: wrote.append('label'))
    table.rows = []
    table.write()
    assert wrote == []


def test_write_table_and_label(monkeypatch, tmp_path):
    table = com.Table(output_dir=FCPath(tmp_path), volume_id='GO_0001',
                      qualifier='body', level='summary')
    calls = []
    monkeypatch.setattr(com.util, 'write_txt_file',
                        lambda path, rows: calls.append(('write', rows)))
    monkeypatch.setattr(com.util, 'dbprint', lambda msg: calls.append(('db', msg)))
    monkeypatch.setattr(com.lab, 'create',
...
def test_write_labels_only_skips_table(monkeypatch, tmp_path):
    table = com.Table(output_dir=FCPath(tmp_path), volume_id='GO_0001',
                      qualifier='inventory', suffix='_inventory.csv',
    ...
    monkeypatch.setattr(com.lab, 'create', lambda *a, **k: calls.append('label'))
    table.rows = []
...
# PathAction
#===============================================================================
def test_path_action_collapses_slashes_preserves_scheme():
...
# get_common_args
#===============================================================================
def test_get_common_args_skips_volume_when_none():
...
def test_get_common_args_flags():
    parser = com.get_common_args(host='GO')
...
# task list
#===============================================================================
def test_add_task_appends(monkeypatch):
...
def test_task_source_yields_tasks(monkeypatch):
    monkeypatch.setattr(com, 'get_logger',
                        lambda: types.SimpleNamespace(info=lambda *a, **k: None))
...
def test_write_task_file_writes_json(monkeypatch, tmp_path):
    monkeypatch.setattr(com, 'get_logger',
                        lambda: types.SimpleNamespace(info=lambda *a, **k: None))
    com.add_task('GO_0001', 'index')
...
def test_write_task_file_noop_without_path():
    assert com.write_task_file(None) is None


#===============================================================================
# init_logger
#===============================================================================
def test_init_logger_registers_handlers(monkeypatch, tmp_path):
    handlers = []
    ...
    monkeypatch.setattr(com.pdslogger, 'file_handler',
                        lambda path, level: ('file', path))
    monkeypatch.setattr(com.pdslogger, 'STDOUT_HANDLER', ('stdout',))
