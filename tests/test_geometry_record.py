...
# get_backplane_key
#===============================================================================
def test_get_backplane_key_tuple_event_key():
    desc = (('phase_angle', 'IO'), ('', '', ''))
    assert Record.get_backplane_key(desc) == 'phase_angle'


def test_get_backplane_key_plain_event_key():
...
# get_key_map
#===============================================================================
def test_get_key_map_slices_last_ndata_columns(record_stub):
...
def test_get_key_map_caches_per_qualifier(record_stub):
    record = record_stub(backplane_keys={}, dicts={
...
def test_get_key_map_handles_dict_of_named_lists(record_stub):
    record = record_stub(backplane_keys={}, dicts={
...
#===============================================================================
def test_postprocess_propagates_null_across_linked_columns(record_stub):
    record = record_stub(backplane_keys={}, dicts={
        'body': [(('center_coordinate', 'IO', 'u'), ('', '', '')),
                 (('center_coordinate', 'IO', 'v'), ('', '', ''))]})
...
def test_postprocess_leaves_non_null_linked_columns(record_stub):
    record = record_stub(backplane_keys={}, dicts={
        'body': [(('center_coordinate', 'IO', 'u'), ('', '', '')),
                 (('center_coordinate', 'IO', 'v'), ('', '', ''))]})
...
#===============================================================================
def test_obs_excluded_empty_is_false():
    record = types.SimpleNamespace(observation=object())
    assert bodies_select.obs_excluded(record, []) is False


def test_obs_excluded_regex_match(monkeypatch):
    monkeypatch.setattr(util, 'get_observation_id', lambda obs: 'C0123CAL')
    record = types.SimpleNamespace(observation=object())
    assert bodies_select.obs_excluded(record, ['.*CAL']) is True


def test_obs_excluded_identifier_calls_config_function(monkeypatch):
...
#===============================================================================
def test_get_primary_in_range(monkeypatch):
    monkeypatch.setattr(util, 'sclk_to_ticks', lambda sclk, sc: 150)
    table = [((100, 200), [], 'JUPITER', ['IO'], ['EUROPA'], ['ADRASTEA'])]
    record = types.SimpleNamespace(observation=object())
    assert bodies_select.get_primary(record, table, '150') == \
        ('JUPITER', ['IO'], ['EUROPA'], ['ADRASTEA'])


def test_get_primary_no_match_returns_fail(monkeypatch):
    monkeypatch.setattr(util, 'sclk_to_ticks', lambda sclk, sc: 999)
    table = [((100, 200), [], 'JUPITER', ['IO'], [], [])]
    record = types.SimpleNamespace(observation=object())
    assert bodies_select.get_primary(record, table, '999') == ('', [], [], [])


def test_get_primary_excluded_observation_short_circuits(monkeypatch):
    monkeypatch.setattr(util, 'sclk_to_ticks', lambda sclk, sc: 150)
    monkeypatch.setattr(util, 'get_observation_id', lambda obs: 'C0CAL')
    table = [((100, 200), ['.*CAL'], 'JUPITER', ['IO'], [], [])]
    record = types.SimpleNamespace(observation=object())
    assert bodies_select.get_primary(record, table, '150') == ('', [], [], [])
