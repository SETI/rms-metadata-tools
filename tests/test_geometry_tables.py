...
# Table.add dispatch
#===============================================================================
def test_inventory_table_add_formats_prefixes_and_list():
...
def test_sky_table_add_uses_no_body():
...
def test_ring_table_add_only_when_rings_present():
...
def test_ring_table_add_skips_without_rings():
    table = tables.RingTable(level='summary')
...
def test_ring_table_add_skips_without_primary():
    table = tables.RingTable(level='summary')
...
def test_body_table_add_iterates_bodies():
...
def test_record_add_builds_summary_line(record_stub, monkeypatch):
...
def test_record_add_named_column_dict(record_stub, monkeypatch):
    monkeypatch.setattr(oops.Body, 'exists', staticmethod(lambda name: True))
    record = record_stub(
        pointing_available=True, sampling=8, backplane_keys={}, blocker=None,
        primary='JUPITER', prefixes=['"vol"', '"file"'], backplane=_Backplane(),
...
#===============================================================================
def _override_record(record_stub):
...
def test_suite_get_override_builds_one_dict_per_column(record_stub):
...
def test_suite_get_override_named(record_stub):
    record = _override_record(record_stub)
...
def test_suite_get_overrides_covers_sky_ring_body(record_stub):
    record = _override_record(record_stub)
    overrides = Suite.get_overrides(record)
    assert set(overrides) == {'sky', 'ring', 'body'}


def test_suite_add_tables_creates_four_tables(record_stub):
    suite = Suite.__new__(Suite)
    suite.template_path = None
    suite.volume_id = 'GO_0001'
    suite.add_tables(None, 'summary')
    qualifiers = [t.qualifier for t in suite.tables]
    assert qualifiers == ['inventory', 'sky', 'ring', 'body']


def test_suite_make_records_one_per_level(monkeypatch):
    suite = Suite.__new__(Suite)
    ...
    created = []
    monkeypatch.setattr(suite_mod, 'Record',
                        lambda *args: created.append(args[-1]) or args[-1])
    records = suite.make_records(0)
    assert records == ['summary', 'detailed']


def test_suite_add_dispatches_by_level():
    suite = Suite.__new__(Suite)
    sky = types.SimpleNamespace(level='summary', added=[],
                                add=lambda r: sky.added.append(r))
    inv = types.SimpleNamespace(level=None, added=[],
...
def test_suite_write_calls_each_table(monkeypatch):
    suite = Suite.__new__(Suite)
    written = []
    suite.tables = [types.SimpleNamespace(write=lambda labels_only: written.append(labels_only))]
    suite.write(labels_only=True)
    assert written == [True]


def test_suite_create_returns_early_without_observations():
    suite = Suite.__new__(Suite)
    # No 'observations' attribute -> create() returns immediately.
    assert suite.create() is None


def test_suite_create_processes_observations(monkeypatch):
    suite = Suite.__new__(Suite)
    ...
    monkeypatch.setattr(suite_mod.config, 'cleanup', lambda: None, raising=False)
    monkeypatch.setattr(suite_mod.com, 'get_logger',
...
def test_suite_create_skips_glob_mismatch(monkeypatch):
    suite = Suite.__new__(Suite)
