...
def test_write_terminator_none_infers_crlf_from_list(tmp_path):
...
def test_append_to_existing_file_grows_once(tmp_path):
...
def test_append_to_existing_terminator_none_infers_from_list(tmp_path):
...
# expandvars
#===============================================================================
def test_expandvars_expands_and_preserves_str(monkeypatch):
...
def test_expandvars_preserves_scheme_and_fcpath_type(monkeypatch):
    monkeypatch.setenv('BUCKET', 'my-bucket')
    result = util.expandvars(FCPath('gs://$BUCKET/x'))
    assert result.as_posix() == 'gs://my-bucket/x'
    assert isinstance(result, FCPath)


def test_expandvars_preserves_path_type(monkeypatch):
