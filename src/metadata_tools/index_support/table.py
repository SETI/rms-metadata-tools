################################################################################
# table.py - IndexTable class for building supplemental index tables.
################################################################################
"""IndexTable class for building supplemental index tables."""
import ast
import fnmatch
...
class IndexTable(com.Table):
    ...
    def _index_one_value(self,
        ...
        # Resolve the key function: a built-in `key__<name>` (from key_fns) takes
        # priority over one defined in the index_config module. If neither exists,
        # the value is taken straight from the label. Look the function up explicitly
        # (rather than catching KeyError/AttributeError) so that an error raised
        # *inside* a key function propagates instead of being silently swallowed.
